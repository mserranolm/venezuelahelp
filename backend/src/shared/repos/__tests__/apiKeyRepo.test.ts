import { describe, it, expect, beforeEach } from "vitest";
import { createHash } from "node:crypto";
import { mockClient } from "aws-sdk-client-mock";
import {
  DynamoDBDocumentClient,
  GetCommand,
  PutCommand,
  QueryCommand,
  UpdateCommand,
} from "@aws-sdk/lib-dynamodb";
import { ApiKeyRepo } from "@/shared/repos/apiKeyRepo";

const ddbMock = mockClient(DynamoDBDocumentClient);
beforeEach(() => ddbMock.reset());

describe("ApiKeyRepo", () => {
  it("create returns a raw key once and stores only its sha256 as SK", async () => {
    ddbMock.on(PutCommand).resolves({});
    const { rawKey, apiKey } = await new ApiKeyRepo().create({
      consumerName: "Cruz Roja",
      email: "datos@cruzroja.org",
      requestId: "req-1",
      createdAt: "2026-06-29T00:00:00.000Z",
    });

    expect(rawKey).toMatch(/^vh_live_/);
    expect(apiKey.status).toBe("active");
    expect(apiKey.keyId).toBeTruthy();

    const item = ddbMock.commandCalls(PutCommand)[0].args[0].input.Item as Record<
      string,
      unknown
    >;
    const hash = createHash("sha256").update(rawKey).digest("hex");
    expect(item.PK).toBe("APIKEY");
    expect(item.SK).toBe(hash);
    // El valor en claro NUNCA se persiste.
    expect(JSON.stringify(item)).not.toContain(rawKey);
  });

  it("getByHash reads by PK=APIKEY / SK=hash (O(1) hot path del authorizer)", async () => {
    ddbMock.on(GetCommand).resolves({
      Item: {
        PK: "APIKEY",
        SK: "deadbeef",
        keyId: "k1",
        consumerName: "X",
        email: "x@x.com",
        requestId: "r1",
        status: "active",
        createdAt: "2026-06-29T00:00:00.000Z",
      },
    });
    const got = await new ApiKeyRepo().getByHash("deadbeef");
    expect(got?.keyId).toBe("k1");
    const key = ddbMock.commandCalls(GetCommand)[0].args[0].input.Key;
    expect(key).toEqual({ PK: "APIKEY", SK: "deadbeef" });
  });

  it("list uses a Query on the shared partition (NOT a full-table Scan)", async () => {
    ddbMock.on(QueryCommand).resolves({
      Items: [
        {
          PK: "APIKEY",
          SK: "h1",
          keyId: "k1",
          consumerName: "X",
          email: "x@x.com",
          requestId: "r1",
          status: "active",
          createdAt: "2026-06-29T00:00:00.000Z",
        },
      ],
    });
    const all = await new ApiKeyRepo().list();
    expect(all.map((k) => k.keyId)).toEqual(["k1"]);
    const input = ddbMock.commandCalls(QueryCommand)[0].args[0].input;
    expect(input.KeyConditionExpression).toContain("PK = :pk");
    expect(input.ExpressionAttributeValues).toMatchObject({ ":pk": "APIKEY" });
  });

  it("revoke finds the key by keyId (Query) and sets status revoked", async () => {
    // DynamoDB aplica el FilterExpression (keyId=k2) server-side sobre la
    // partición, así que el Query ya devuelve solo el match.
    ddbMock.on(QueryCommand).resolves({
      Items: [{ PK: "APIKEY", SK: "h2", keyId: "k2", status: "active" }],
    });
    ddbMock.on(UpdateCommand).resolves({});
    const ok = await new ApiKeyRepo().revoke("k2", "2026-06-29T02:00:00.000Z");
    expect(ok).toBe(true);
    const input = ddbMock.commandCalls(UpdateCommand)[0].args[0].input;
    expect(input.Key).toEqual({ PK: "APIKEY", SK: "h2" });
    expect(input.ExpressionAttributeValues).toMatchObject({
      ":status": "revoked",
      ":revokedAt": "2026-06-29T02:00:00.000Z",
    });
  });

  it("revoke returns false when the keyId is unknown", async () => {
    ddbMock.on(QueryCommand).resolves({ Items: [] });
    const ok = await new ApiKeyRepo().revoke("nope", "2026-06-29T02:00:00.000Z");
    expect(ok).toBe(false);
    expect(ddbMock.commandCalls(UpdateCommand)).toHaveLength(0);
  });
});
